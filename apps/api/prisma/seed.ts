...
async function main() {
  ...
  // Units — Brickell Apartments (4 units)
  const unit1a = await prisma.unit.create({
    data: { propertyId: prop1.id, unitNumber: '1A', bedrooms: 2, bathrooms: 2, sqft: 950, rentAmountCents: 2800_00 },
  });
  const unit1b = await prisma.unit.create({
    data: { propertyId: prop1.id, unitNumber: '1B', bedrooms: 1, bathrooms: 1, sqft: 650, rentAmountCents: 2100_00 },
  });
  const unit2a = await prisma.unit.create({
    data: { propertyId: prop1.id, unitNumber: '2A', bedrooms: 2, bathrooms: 2, sqft: 950, rentAmountCents: 2900_00, status: 'OCCUPIED' },
  });
  const unit2b = await prisma.unit.create({
    data: { propertyId: prop1.id, unitNumber: '2B', bedrooms: 1, bathrooms: 1, sqft: 650, rentAmountCents: 2200_00, status: 'OCCUPIED' },
  });

  // Units — South Beach Villa (1 unit = whole property)
  const unitSB = await prisma.unit.create({
    data: { propertyId: prop2.id, unitNumber: 'MAIN', bedrooms: 3, bathrooms: 2, sqft: 1800, rentAmountCents: 350_00 },
  });

  // Units — Little Havana Fourplex (4 units)
  const unitLH1 = await prisma.unit.create({
    data: { propertyId: prop5.id, unitNumber: '1', bedrooms: 2, bathrooms: 1, sqft: 750, rentAmountCents: 1800_00, status: 'OCCUPIED' },
  });
  const unitLH2 = await prisma.unit.create({
    data: { propertyId: prop5.id, unitNumber: '2', bedrooms: 2, bathrooms: 1, sqft: 750, rentAmountCents: 1800_00, status: 'OCCUPIED' },
  });
  const unitLH3 = await prisma.unit.create({
    data: { propertyId: prop5.id, unitNumber: '3', bedrooms: 1, bathrooms: 1, sqft: 550, rentAmountCents: 1500_00 },
  });
  const unitLH4 = await prisma.unit.create({
    data: { propertyId: prop5.id, unitNumber: '4', bedrooms: 1, bathrooms: 1, sqft: 550, rentAmountCents: 1500_00, status: 'MAINTENANCE' },
  });
  ...
  const lease1 = await prisma.lease.create({
    data: {
      ...
      rentAmountCents: 2900_00,
      depositCents: 2900_00,
    },
  });
  ...
  const lease2 = await prisma.lease.create({
    data: {
      ...
      rentAmountCents: 2200_00,
      depositCents: 2200_00,
    },
  });

  // Payments for lease1 (Jan–Apr 2026)
  ...
  for (const m of months) {
    await prisma.payment.create({
      data: {
        ...
        amountCents: 2900_00,
        dueDate: new Date(m),
        paidAt: new Date(new Date(m).getTime() + 2 * 86400000),
        method: 'ach',
      },
    });
  }
  ...
  // Reservations — South Beach Villa
  await prisma.reservation.create({
    data: {
      ...
      guestEmail: 'mbrown@gmail.com',
      guestPhone: '(646) 555-0500',
      channel: 'airbnb',
      externalId: 'ABB-892341',
      status: 'CHECKED_OUT',
      checkIn: new Date('2026-04-10'),
      checkOut: new Date('2026-04-17'),
      nightlyRateCents: 450_00,
      totalCents: 3150_00 + 200_00,
      cleaningFeeCents: 200_00,
    },
  });
  ...
  const upcomingRes = await prisma.reservation.create({
    data: {
      ...
      guestEmail: 'emma.w@outlook.com',
      channel: 'vrbo',
      externalId: 'VRBO-441289',
      status: 'CONFIRMED',
      checkIn: new Date('2026-05-05'),
      checkOut: new Date('2026-05-12'),
      nightlyRateCents: 475_00,
      totalCents: 3325_00 + 200_00,
      cleaningFeeCents: 200_00,
    },
  });
  ...
  await prisma.reservation.create({
    data: {
      ...
      unitId: unitSB.id,
      guestName: 'David Kim',
      guestEmail: 'dkim@yahoo.com',
      channel: 'direct',
      status: 'CONFIRMED',
      checkIn: new Date('2026-05-20'),
      checkOut: new Date('2026-05-25'),
      nightlyRateCents: 500_00,
      totalCents: 2500_00 + 200_00,
      cleaningFeeCents: 200_00,
    },
  });
  ...
  await prisma.cleaningSchedule.create({
    data: {
      ...
      notes: 'Deep clean before guest check-in',
    },
  });
  console.log('  Created 1 cleaning schedule');
  ...
  await prisma.maintenanceJob.create({
    data: {
      ...
      description: 'The kitchen faucet drips constantly even when fully closed. Getting worse over the past week.',
      priority: 'MEDIUM',
      status: 'IN_PROGRESS',
      category: 'plumbing',
      estimateCents: 250_00,
  ...
  await prisma.maintenanceJob.create({
    data: {
      ...
      description: 'Full unit turnover: paint touch-up, deep clean, replace carpet in bedroom, fix bathroom exhaust fan.',
      priority: 'HIGH',
      status: 'OPEN',
      category: 'general',
      estimateCents: 1200_00,
    },
  });
  ...
  await prisma.maintenanceJob.create({
    data: {
      ...
      description: 'AC unit is running but not producing cold air. Thermostat shows 85°F with set temp at 72°F.',
      priority: 'EMERGENCY',
      status: 'IN_PROGRESS',
      category: 'hvac',
      estimateCents: 800_00,
  ...
  await prisma.maintenanceJob.create({
    data: {
      organizationId: org.id,
      propertyId: prop2.id,
      createdById: cleaner.id,
      title: 'Pool pump making noise',
      description: 'The pool pump is making a grinding sound. May need bearing replacement.',
      priority: 'LOW',
      status: 'OPEN',
      category: 'general',
    },
  ...
  // Renovation — Wynwood Duplex
  const reno = await prisma.renovation.create({
    data: {
      propertyId: prop3.id,
      name: 'Full Gut Renovation',
      status: 'IN_PROGRESS',
      budgetCents: 180000_00,
      actualCostCents: 87500_00,
      startDate: new Date('2025-02-01'),
      endDate: new Date('2026-08-31'),
      notes: 'Converting to modern 2-unit duplex. New electrical, plumbing, roof, kitchen, bathrooms.',
    },
  });
  ...
  console.log('  Created renovation with 8 expense line items');

  // Listing — Coral Gables Townhouse
  await prisma.listing.create({
    data: {
      propertyId: prop4.id,
      status: 'ACTIVE',
      askingPriceCents: 725000_00,
      description: 'Beautiful 3BR/2BA townhouse in the heart of Coral Gables. Updated kitchen, hardwood floors, 2-car garage. Walking distance to Miracle Mile.',
      publishedAt: new Date('2026-04-15'),
    },
  });
  console.log('  Created 1 active listing');

  // Transactions — mix of income and expenses
  const txData = [
    // Rent income
  ...
  for (const tx of txData) {
    await prisma.transaction.create({
      data: { organizationId: org.id, ...tx },
    });
  }
  console.log('  Created 19 transactions (10 income, 9 expenses)');

  // Documents
  await prisma.document.createMany({
    data: [
      { organizationId: org.id, propertyId: prop1.id, leaseId: lease1.id, name: 'Lease Agreement - James Williams - Unit 2A.pdf', fileUrl: '/documents/lease-2a.pdf', mimeType: 'application/pdf', sizeBytes: 245000 },
      { organizationId: org.id, propertyId: prop1.id, leaseId: lease2.id, name: 'Lease Agreement - Lisa Chen - Unit 2B.pdf', fileUrl: '/documents/lease-2b.pdf', mimeType: 'application/pdf', sizeBytes: 238000 },
      { organizationId: org.id, propertyId: prop4.id, name: 'Property Inspection Report - Coral Gables.pdf', fileUrl: '/documents/inspection-cg.pdf', mimeType: 'application/pdf', sizeBytes: 1250000 },
      { organizationId: org.id, propertyId: prop3.id, name: 'Renovation Plans - Wynwood Duplex.pdf', fileUrl: '/documents/reno-plans.pdf', mimeType: 'application/pdf', sizeBytes: 5400000 },
    ],
  });
  ...
  console.log('  Owner:    meni@casameni.com / password123');
  console.log('  Manager:  sarah@casameni.com / password123');
  console.log('  Tech:     carlos@casameni.com / password123');
  console.log('  Cleaner:  maria@casameni.com / password123');
  console.log('  Tenant:   james@tenant.com / password123');
  console.log('  Tenant:   lisa@tenant.com / password123');
  console.log('  Acct:     david@casameni.com / password123');
}
